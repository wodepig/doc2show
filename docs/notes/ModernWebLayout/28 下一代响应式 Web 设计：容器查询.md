...
![img](./img/18f52d4afa58406b8dcb466de9ca9605~tplv-k3u1fbpfcp-zoom-1.png)

通过前面课程的学习，我们知道了。在 CSS 中，Web 开发者可以根据 CSS 媒体查询特性（通常是视窗宽度、媒体设备特性等）来为 Web 页面定制不同的表现形式，比如可以根据用户浏览内容的设备特性来呈现不同的布局、字体大小和图片等。

![img](./img/a049e0660b06465a8d50e8badaa5d0b2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/15fb2c4de8004eada8d8283406fdcc02~tplv-k3u1fbpfcp-zoom-1.png)

其实，自从我们在 2021 年 4 月第一次看到容器查询原型以来，它的语法已经更改了几次。庆幸的是，现在容器查询的规范也稳定了，浏览器也准备发布了！现在使用，你也不必担心其语法规则的变化了。

![img](./img/d00258457baf49eba863a203afc1ae64~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/c4188c738c2445969576ad058fac47f9~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/c7515be040494766bd8fa2fc12c9f295~tplv-k3u1fbpfcp-zoom-1.png)

简化一下，以卡片 `.card` 组件为例。

![img](./img/d9e5f5a03e3944c58894355da448c3a3~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8520402c0a9d45749c76a20fc8ce791e~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/15e3014cd1d54e148d3f9bbbbb947736~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/160504ca68cc439ea42fcf53c5c5884e~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/jOpMmZa>

![img](./img/0052e9d1703a4b26bedbcff91d7ee06c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/77005055ea1d4283b56b9f059a7d2f26~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8f7f449b50b34c35bb1a0a0a5c3828e0~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/QWBKdzP>

有了这样一个卡片组件之后，如果将其放在不同的位置，即使是同一页面，同一视窗断点下，也会根据其容器断点自动匹配最为适合的布局（或UI效果）。比如：

![img](./img/01f7a34d3b29457bbbe5b6940e9fb554~tplv-k3u1fbpfcp-zoom-1.png)

从上图的效果中不难发现，位于侧边栏 `aside` 的卡片组件，它始终能保持下图呈现：

![img](./img/d4fbe8c546e846d5a20a95302a8de9ab~tplv-k3u1fbpfcp-zoom-1.png)

位于 `main` 栏的卡片将会根据 `.card__container` 的宽度有着不同的方式呈现：

![img](./img/b04f3388dc64487e904a1362b6387f6c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ba0e22a8e4a74d4eab2e19b961f1f251~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ed9e41f698734e13ab414b1d9abce2e9~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/QWBKRmg>

我们前面有提到过，Web 内容输出是动态的，可能因为卡片的扩展或收缩，CSS Flexbox 布局最终呈现的效果会和设计效果不一致：

![img](./img/8c5815e2204442c69a523773f124c9a2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/71c25ebbf4e74fed9283323626190932~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/6276e844b4854f0cbe16fb927ee38933~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a0e63613edb7428389f3ac57cb5d1884~tplv-k3u1fbpfcp-zoom-1.png)

## 设计时考虑容器查询

虽然响应式 Web 设计给 Web 设计师带来了更多的可能性，但响应式 Web 设计还是有很多的局限性。对于 Web 设计师而言，更期待的是能够根据组件容器尺寸来提供不同的设计风格。依旧拿卡片组件来举例：

![img](./img/2dc3d4b806a743a08bf13aa0dfec7ef4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/06fbf92e923442e0998f8c5a59ecabc4~tplv-k3u1fbpfcp-zoom-1.png)

在未来，Web 设计师给 Web 开发者投喂的设计稿可能就会像下图这样了：

![img](./img/00b0103f11fe4861be272bafb3349749~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/e70a570ffee740ddab9c861cdca7288d~tplv-k3u1fbpfcp-zoom-1.png)

作为 Web 开发人员，看到上图设计效果，需要改变以往对设计图意图的理解，不能继续执着于基于视窗尺寸来调整组件 UI。

![img](./img/9094d802c2294cb58da5a9380a84c8d4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/1fd2475dbc974b11a9ac5591bba3c394~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8153b89132914976bcd69b15ffc1f616~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8e2b70eece7a4e62ae39bcbe92774aa2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/4bf0a29068fc4cc0bed7bc0f198078a3~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a13c522ecf5941f6ba53a858d4d495c1~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/bfe193296ef94b0090ef618f691ab311~tplv-k3u1fbpfcp-zoom-1.png)

就拿前面的示例为例。

![img](./img/98b0b34be6d0462291f97e4879a2a4f8~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/c9fd1c66aeae405d91266d8a8829ce99~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/4b86a704901a4540bf082911fa7dc1b7~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/22e4cb8166a3423182bfaa0ed2fc65ec~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8ee84e06935c45e5a3b6f1313a5e6136~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/eaef1633c72d4cc7a0b0b94b64b53548~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a91bcb7022ee474791ff40c7a4aa77f5~tplv-k3u1fbpfcp-zoom-1.png)

像上图这样的效果，我们可以使用 CSS 容器查询来实现。当有足够的空间时，侧边栏的列表会展开，如果没有足够空间时，侧边栏只会展示 Icon 图标（或用户头像）。我们来实现一个像下图的布局效果。

![img](./img/f32211f443d64fb6ae129e7fdb2f88bc~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/445f2685103a4a02a4dda5fc50756b1d~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/70e70650a1b44224b56b3564d2557735~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/6ac1159cf2f44ee7952242d709c23213~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/c2c118fabbe2405e80d87c753319a99a~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/cab59d7f46124dd6b829331e90bff594~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/47a836af66f546ca94e5b2cee223864a~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/20d623b746c742deba944b0dbdcca01c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a3df7e48361e489ebb91680fc9e78453~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/629754c9cf3d423a93a2ead0e710d2b4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/bad77b85ce7244589967b9cff405f842~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/b00300e84bba4508becaaad804280b50~tplv-k3u1fbpfcp-zoom-1.png)

上面示例是容器查询和样式查询组合在一起的。你可能还没有体会到样式查询所起的作用。那我们来看两个纯样式查询的示例。比如下图这个布局效果：

![img](./img/a313b06ad07f45a387f0ec4aace10310~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ebe6ef39a73d473281bb306eda6661d0~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/b92927579bc7436e9b929903d01eb4c4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8114cae1efe94c00a574c4605fc0b410~tplv-k3u1fbpfcp-zoom-1.png)
