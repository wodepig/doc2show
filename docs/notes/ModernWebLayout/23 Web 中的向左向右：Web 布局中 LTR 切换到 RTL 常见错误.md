...
![img](./img/7e1348ba9b8543369fa4489307b214b1~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/4fd2e6bab00a4ef8ac6bf24c35bf867c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/2ef8d81c74564efb8603be3675e9d7b9~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/5348c9b9ad18461cb4c66b7912d64ea2~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/eYKXbJM>

你会发现字母之间有一些不同颜色的区域。看上去是有层叠区域造成的颜色不一致：

![img](./img/067ee94236174271a8a43b1a9ce3d048~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ee52974e06a842f492e11296d3ae2d49~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/9dcccf8b6ed74256b2d6a35c6d7a80db~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/afa6563b45ee4e3d885b7e69fe562595~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/49d2cdbfd9744314bdcda84de2cf784d~tplv-k3u1fbpfcp-zoom-1.png)

针对这样的场景，更好的做法是，使用 CSS 的内在尺寸来定义元素容器的大小，比如可以将按钮设置的宽度为 `auto` 、`min-content` 或 `max-content` ，这样使不同版本语言下都有一个较好的宽度：

![img](./img/92846bcc717d4bb887a4ae4c5c0ddedd~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ae1fcfec712d41138462619a752194d8~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ad475b752c2447da9016175687f944d5~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/cf2951b325664dc692bfa5607a9b2d9f~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址： <https://codepen.io/airen/full/wvXObbN>

上面的效果应该是符合我们预期的。如果不慎在拉丁语体系和汉语体系中设置了 `dir="rtl"` 时，效果和我们阅读习惯就不同了，甚至是一种错误的表现行为：

![img](./img/8ddd739726394acca4b902c3e1411c79~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/bc82b275ab2e4e51852fe39891357551~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a3f1b10b9467464389acdf47bbfeb91e~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/09cd832e91754c4796acceee1ccbf2d2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/9e9f340b2b2a44d0afa46101e7681bf3~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/cb420196539544948a1e10e37a298378~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/NWzJZzG>

另外建议 `line-height` 不要使用固定单位的值，这样在一些语言的切换状态下很容易造成文本展示不全（类似被截）。比如下图的效果：

![img](./img/9e2e33daf82d4302872fd7a3fed15bab~tplv-k3u1fbpfcp-zoom-1.png)

## 不采用默认的文本下划线

有些文本会带有默认下划线的效果，比如 `<a>` 链接。在阿拉伯语言的文本中，默认的文本下划线会让阅读变得很困难。这种现象的产生，与阿拉伯语单词和字母的书写方式有关。如下图所示：

![img](./img/b2c3012ac8474ae8b588f436a714c3bc~tplv-k3u1fbpfcp-zoom-1.png)

你会发现，文本下划线会和一些文本重叠，比如单词中的一些点：

![img](./img/e0b2bac15a6640f4a33f26190345e09f~tplv-k3u1fbpfcp-zoom-1.png)

另外，采用默认的文本下划线，不同的浏览器渲染出的效果也会有所差异：

![img](./img/8c719b0d80c34885b9db07accf2e0ca2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/6dc50e1b46de48d6b6c26ed1f80c5207~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/f2c6b48a214741d48c9af60224abd9ba~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/97d9b4cfa41741dcba27a0603e04899b~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/12f3a52c64b64cf89206e921bef29f5f~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/cc66004cf8dc44bc8f60b16558d4a5a4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/8612f93a821143dcaf265b72e70985a1~tplv-k3u1fbpfcp-zoom-1.png)

像上述这种对称性的图标，用在双向语言中，你不需要对这些图标做任何的处理（比如翻转）。

在双向语言系统中有些图标是具有方向性的。也就是说在 LTR 和 RTL 中要改变它们的方向，而且这一点对于用户来说是非常重要的，可以更清楚地理解图标的含义。比如：

![img](./img/5dcc866547c54645bc7cfb25370d2b10~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/4129022de1f448dbbfc0017c456e97da~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/6bc3fd60d3d24ace8b8ccc288fede697~tplv-k3u1fbpfcp-zoom-1.png)

> Demo 地址：<https://codepen.io/airen/full/ZERZJLr>

也就是说，我们在 LTR 和 RTL 版本中使用图标时，应该尽可能像下图这样来使用：

![img](./img/da9331f9cc8f45a996a8c08f3496e133~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/a00aca7c424140a5b181da162f3f346b~tplv-k3u1fbpfcp-zoom-1.png)

还有一些图标是通用的，也不需要翻转它们。例如，播放器上的一些图标，它代表的是磁带播放的方向，而不是时间方向，所以不必要对它们做翻转。下图是 Spotify 应用程序的英语和阿拉伯语版本:

![img](./img/b9754599e36a4868a6f3e7221e7880b5~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/3aa9bef9a85742b3b1d2601d9b0b014e~tplv-k3u1fbpfcp-zoom-1.png)

对于表单控件也是如此，特别是对于输入型的 `input` 表单控件，还应该保持输入的方向性：

![img](./img/38689fa16b6c4f91977ee6bf4de8e93f~tplv-k3u1fbpfcp-zoom-1.png)

在 RTL 中，有些表单输入应该保持左对齐，例如电子邮件和手机号码。值得注意的是，如果占位符内容是阿拉伯语或其他 RTL 语言，那么占位符应该向右对齐。一旦输入框获得焦点，用户开始输入，对齐方式将翻转到左侧。

![img](./img/35e8ce0ca6b144a1bc3b61f6567bfc0c~tplv-k3u1fbpfcp-zoom-1.png)

### 导航菜单和面包屑

对于导航菜单以及页头，还有面包屑等 UI 的设计在双向语言中是 UI 的镜像。

![img](./img/a1e5fdfcde6248549fe77222ee05b111~tplv-k3u1fbpfcp-zoom-1.png)

### 数字顺序

在双向语言中，对于数字的顺序（比如电话号码、门牌号等），不需要做镜像的处理。但要是带有图标的话，对应的图标还是需要做镜像处理的。

![img](./img/6df6c713914d47ffa94611c0733a6147~tplv-k3u1fbpfcp-zoom-1.png)

### 组件的翻转

在处理一些组件时，我需要一种快速翻转它们的方法。在 Sketch 应用中，我将复制一个组件，然后用 “flip” 命令翻转它。同样的功能也可以在 Adobe XD 和 Figma 中使用。

![img](./img/0a49dce378334638bab49064e31965a9~tplv-k3u1fbpfcp-zoom-1.png)

而我们在 Web 中构建 Web 组件时，大部分通过 HTML 的 `dir` 或 CSS 的 `direction` 就可以实现水平翻转的效果：

![img](./img/7505e9a0031e42f6b7c0879db2bc396b~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/615ee46892184646ad7d8290413fbdd0~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/d6bf994ae90e404f96115e5ca883e461~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/86c37e6705624b2584245aa0c8953753~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/fd448b72e89d4d4f8369a07a08197bf1~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/6a1a9cefa1284576b43a89b73e6f110d~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/7ad5441032c041f8b911688e2fdf0d1c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/b519df2a5d374a2888ef71d651e7bd2c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/4084fa227c0241eda9e1a50b096ba069~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/20dfbdef477b43bbb73a1479766d43a2~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/ca1b960039eb4e4794128dd9d064e394~tplv-k3u1fbpfcp-zoom-1.png)
...
![fig-56.jpg](./img/f96f2c59f406455aba903a44c95f894d~tplv-k3u1fbpfcp-watermark.png)
...
![fig-57.jpg](./img/debdcfb8434c40928049790caba515e8~tplv-k3u1fbpfcp-watermark.png)


最后，要是你对 CSS 逻辑属性和物理属性之间的对应关系记不住，也不要紧，查看下图的即可：

![img](./img/a55b66f8f6174c1589c7b9d6cd8a3f3d~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/aa53b1d197b04a18a83a98eb706dfb74~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/155a87a3b565458591676b7922ddb292~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/65b4328de036454ba357feb9427bd35a~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/dd1df52b7559421c90c344eec32c0b04~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/7742ba2cfb954d48815c077b5036ee37~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/7f984aba816347139ada810a65e06c0c~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/2a6a9fd22dea47e7afabf2173e43508a~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/0aea3e5ca7ac4c7884b7065a07b7e0cd~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/9b911556865b4df397857cef9e1c8650~tplv-k3u1fbpfcp-zoom-1.png)

我在 Fork 的示例基础上对按钮做了一个最小尺寸的设置，这样做的主要原因是，在不同语言版本时，不会让有的语言版本下视觉看上去不美观，也避免按钮不好点击：

![img](./img/309099ed834843fbb778cc2cf5bbc2f4~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./img/c1984c5b18284bf6a0048a29282074dd~tplv-k3u1fbpfcp-zoom-1.png)
